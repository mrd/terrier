...
extern fun rs1 {l: addr} {a: t@ype} {n: nat} (
    pf_ms: !mslot_v (l, n, a, false) | ms: ptr l
  ): [OP, IP: bit] [WS3occ: nat] (rs1_v (WS3occ, OP, IP) | void)
  = "mac#_rs1"

absview rs2_v (WS3occ: int, OP: bit, IP: bit, Rip: bit, Wip: bit, overlap: bool)
extern fun rs2 {l: addr} {a: t@ype} {OP, IP: bit} {n, WS3occ: nat} (
    pf_ms: !mslot_v (l, n, a, false),
...
extern fun _rs3 {a:t@ype}
                {l1, l2: addr}
                {overlap: bool} {OP, IP, Rip, Wip: bit | (OP != IP) || (Rip != Wip) || (overlap == false)} {WS3occ: nat} (
    pf_rs2: rs2_v (WS3occ, OP, IP, Rip, Wip, overlap) |
    ms: ptr l1, dest: &a? >> a, size: size_t (sizeof a)
  ): void = "mac#_rs3"

fun{a:t@ype} rs3 {l: addr} {overlap: bool} {OP, IP, Rip, Wip: bit | (OP != IP) || (Rip != Wip) || (overlap == false)} {n, WS3occ: nat} (
    pf_ms: !mslot_v (l, n, a, false),
    pf_rs2: rs2_v (WS3occ, OP, IP, Rip, Wip, overlap) |
    ms: ptr l
  ): a = item where {
  var item: a?
  val _ = _rs3 (pf_rs2 | ms, item, sizeof<a>)
}

implement{a} multislot_read (pf_ms | ms) = let
  val (pf_rs1 | _) = rs1 (pf_ms | ms)
  val (pf_rs2 | _) = rs2 (pf_ms, pf_rs1 | ms)
in
  rs3<a> (pf_ms, pf_rs2 | ms)
end
...
implement{a} multislot_initialize_reader (pf | p, pages) =
  multislot_initialize_reader_c (pf | p, pages, sizeof<a>)
...
extern fun ws2 {l: addr} {a: t@ype} {n: nat} (
    pf_ms: !mslot_v (l, n, a, true) | ms: ptr l
