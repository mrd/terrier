...
if [ ! -e "$2" ]; then
        echo "$flash_image_name does not exist. Creating it. Please wait."
...
put_xloader()
{
        echo "X-loader image name: $xloader_image_name"
        put_no_oob $1 $xloader_page_offset
        echo "Wrote X-loader into flash image."
}
put_uboot()
{
        echo "U-boot image name: $uboot_image_name"
        put_no_oob $1 $uboot_page_offset
        echo "Wrote U-boot into flash image."
}
put_kernel()
{
        echo "Kernel image name: $kernel_image_name"
        put_no_oob $1 $kernel_page_offset
        echo "Wrote Kernel into flash image."
}
put_rootfs()
{
        echo "Root FS image name: $rootfs_image_name"
        put_no_oob $1 $rootfs_page_offset
        echo "Wrote Root FS into flash image."
}
case "$3" in
        x-loader)
                put_xloader $1
